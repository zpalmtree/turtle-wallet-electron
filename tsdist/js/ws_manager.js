...
WalletShellManager.prototype.isRunning = function () {
    ...
    childProcess.exec(cmd, (err, stdout, stderr) => {
        ...
        if (stderr)
            log.debug(stderr.toLocaleLowerCase());
...
WalletShellManager.prototype._spawnService = function (walletFile, password, onError, onSuccess, onDelay) {
    ...
    let logFile = path.join(path.dirname(walletFile), `${file.split(' ').join('').split('.')[0]}.log`);
    ...
    function testConnection(retry) {
        wsm.serviceApi.getAddress().then((address) => {
            ...
            if (!serviceDown && retry === 10 && onDelay)
                onDelay(`Still no response from ${config.walletServiceBinaryFilename}, please wait a few more seconds...`);
...
WalletShellManager.prototype.terminateService = function (force) {
    ...
    try {
        ...
        if (this.servicePid)
            process.kill(this.servicePid, signal);
    }
    catch (e) {
...
WalletShellManager.prototype.startSyncWorker = function () {
    ...
    this.syncWorker = childProcess.fork(path.join(__dirname, './ws_syncworker.js'));
...
WalletShellManager.prototype.createWallet = function (walletFile, password) {
    ...
    return new Promise((resolve, reject) => {
        ...
        childProcess.execFile(wsm.serviceBin, serviceArgs, (error, stdout, stderr) => {
            if (stdout)
                log.debug(stdout);
            if (stderr)
                log.debug(stderr);
...
WalletShellManager.prototype._fusionSendTx = function (threshold, counter) {
    ...
    return new Promise((resolve, reject) => {
        ...
        let maxIter = 256;
        if (counter >= maxIter)
            return resolve(wsm.fusionTxHash); // stop at max iter
        wtime(2400).then(() => {
...
WalletShellManager.prototype.optimizeWallet = function () {
    ...
    return new Promise((resolve, reject) => {
        ...
        wsm._fusionGetMinThreshold().then((res) => {
            ...
            return resolve(wsm._fusionSendTx(res).then(() => {
                wsm.notifyUpdate({
                    type: 'fusionTxCompleted',
