...
exports.genQrDataUrl = (inputStr) => {
    if (!inputStr)
        return '';
    let qrtmp = qr.imageSync(inputStr, { type: 'png' });
...
exports.validateMnemonic = (seed) => {
    if (!seed)
        return false;
    if (!MNEMONIC_SEED_REGEX.test(seed))
        return false;
...
exports.validateWalletPath = (fullpath, defaultDir, isExisting) => {
    return new Promise((resolve, reject) => {
        ...
        if (!fullpath.length)
            return reject(new Error('Wallet file path can not be left blank'));
