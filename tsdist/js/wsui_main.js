...
function switchTab() {
    ...
    if (!isServiceReady) {
        ...
        if (nextSection === 'section-overview')
            nextSection = 'section-welcome';
    }
...
function changeSection(sectionId, targetRedir) {
    ...
    if (section.classList.contains('is-shown')) {
        if (toastMsg.length && !targetRedir)
            wsutil.showToast(toastMsg);
        return;
    }
    // reset quick filters
    ...
    if (activeSection)
        activeSection.classList.remove('is-shown');
...
function initNodeSelection(nodeAddr) {
    ...
    if (forceNew)
        settings.set('node_address', nodeAddr);
...
function initSettingVal(values) {
    ...
    if (!settings.has('pubnodes_custom'))
        settings.set('pubnodes_custom', []);
...
function showInitialPage() {
    ...
    if (versionInfo)
        versionInfo.innerHTML = WS_VERSION;
...
function initAddressCompletion(data) {
    ...
    try {
        if (window.COMPLETION_ADDRBOOK)
            window.COMPLETION_ADDRBOOK.destroy();
    }
    catch (e) {
        console.log(e);
    }
            ...
            if (err) {
                ...
                setTimeout(() => {
                    ...
                    fs.rename(oldAddressBook, oldAddressBook + '.deprecated.txt', (err) => {
                        if (err)
                            console.error('Failed to rename old addressbook');
                    });
                }, 500);
                return newBook;
            }
...
function handleWalletOpen() {
    ...
    wsutil.liveEvent("#saveCustomNode", "click", (e) => {
        ...
        if (net.isIPv4(nodeAddress[0]))
            validHost = true;
        if (!validHost) {
            let domRe = new RegExp(/([a-z])([a-z0-9]+\.)*[a-z0-9]+\.[a-z.]+/i);
            if (domRe.test(nodeAddress[0]))
                validHost = true;
        }
        if (!validHost) {
    ...
    walletOpenRefreshNodes.addEventListener('click', () => {
        ...
        if (!confirm("Refreshing node list may take a while to complete, are you sure?"))
            return;
        fetchNodeInfo(true);
    });
    ...
    mox.addEventListener('click', function (e) {
        ...
        if (eid === 'fake-selected-node' || eid === 'fake-select')
            return;
        if (isChild(e.target, walletOpenSelectBox))
            return;
        walletOpenSelectOpts.classList.add('hidden');
    });
}
...
function handleWalletClose() {
    overviewWalletCloseButton.addEventListener('click', (event) => {
        event.preventDefault();
        if (!confirm('Are you sure want to close your wallet?'))
            return;
...
function handleWalletCreate() {
    overviewButtonCreate.addEventListener('click', () => {
        ...
        wsutil.validateWalletPath(filePathValue, DEFAULT_WALLET_PATH).then((finalPath) => {
            ...
            // create
            wsmanager.createWallet(finalPath, passwordValue).then((walletFile) => {
                settings.set('recentWallet', walletFile);
...
function handleWalletImportKeys() {
    importKeyButtonImport.addEventListener('click', () => {
        ...
        wsutil.validateWalletPath(filePathValue, DEFAULT_WALLET_PATH).then((finalPath) => {
            ...
            wsmanager.importFromKeys(finalPath, // walletfile
            passwordValue, viewKeyValue, spendKeyValue, scanHeightValue).then((walletFile) => {
                settings.set('recentWallet', walletFile);
                walletOpenInputPath.value = walletFile;
                changeSection('section-overview-load');
...
function handleWalletImportSeed() {
    importSeedButtonImport.addEventListener('click', () => {
        ...
        wsutil.validateWalletPath(filePathValue, DEFAULT_WALLET_PATH).then((finalPath) => {
            ...
            wsmanager.importFromSeed(finalPath, passwordValue, seedValue, scanHeightValue).then((walletFile) => {
                settings.set('recentWallet', walletFile);
                walletOpenInputPath.value = walletFile;
                changeSection('section-overview-load');
                wsutil.showToast('Wallet have been imported, you can now open your wallet!', 12000);
...
function handleWalletExport() {
    overviewShowKeyButton.addEventListener('click', () => {
        formMessageReset();
        if (!overviewWalletAddress.value)
            return;
...
function handleSendTransfer() {
    ...
    sendButtonSend.addEventListener('click', () => {
        ...
        if (parseFloat(txTotal) > parseFloat(availableBalance)) {
            formMessageSet('send', 'error', `Sorry, you don't have enough funds to process this transfer. Transfer amount+fees: ${(txTotal)}`);
            return;
        }
        // todo: adjust decimal
        let tx = {
    ...
    sendOptimize.addEventListener('click', () => {
        ...
        if (!confirm('You are about to perform wallet optimization. This process may take a while to complete, are you sure?'))
            return;
            ...
            let columnDefs = [
                {
                    ...
                    getQuickFilterText: function (params) {
                        let txdate = new Date(params.data.timeStr).toLocaleString('en-US', { weekday: 'long', year: 'numeric', month: 'long', timeZone: 'UTC' });
...
function initHandlers() {
    ...
    function handleBrowseButton(args) {
        ...
        if (tbtn.classList.contains('d-opened'))
            return;
        ...
        if (dialogType === 'saveFile') {
            dialogOpts.defaultPath = path.join(recentDir, `new_wallet_${(new Date()).getTime()}.${config.walletFileDefaultExt}`);
